...
class Grid:
        ...
        if create:
            ...
            if not os.path.exists(readme_path):
                with open(readme_path, "w") as f:
                    f.write(
                        "# Grids Directory\n\n"
                        "This folder contains horizontal grid definitions generated by "
                        "`mom6_bathy.Grid`.\n\n"
                        "Each grid subfolder includes:\n"
                        "- Supergrid and MOM6 grid files (`grid_*.nc`, `grid_*.json`)\n"
                        "- Metadata such as resolution, domain size, and boundary conditions\n\n"
                        "These grids are used to configure the horizontal layout of MOM6 "
                        "simulations. Do not edit them manually.\n"
                    )
    ...
    def to_netcdf(self, path=None, format="grid"):
        ...
        folder = self._get_grid_folder(root_dir=None)
        os.makedirs(folder, exist_ok=True)
