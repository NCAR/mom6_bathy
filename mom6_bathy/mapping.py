...
def get_smoothed_map_filepath(mapping_file_prefix, output_dir, rmax, fold):
    ...
    """
    output_dir = Path(output_dir)
    nnsm_map_filepath = output_dir / f"{mapping_file_prefix}_r{int(rmax)}_f{int(fold)}_nnsm.nc"
...
def gen_rof_maps(rof_mesh_path, ocn_mesh_path, output_dir, mapping_file_prefix, rmax=None, fold=None):
    ...
    if rmax is not None:
        ...
        print(f"  This may take a while depending on the mesh sizes and smoothing parameters:")
        ...
        nnsm_map_filepath = get_smoothed_map_filepath(mapping_file_prefix, output_dir, rmax, fold)
