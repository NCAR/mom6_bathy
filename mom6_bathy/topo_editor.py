...
class TopoEditor(widgets.HBox):
    ...
    def construct_control_panel(self):
        ...
        self._selected_cell_label = widgets.Label(
            "Selected cell: None (double click on canvas to select a cell)."
        )
    ...
    def refresh_display_mode(self, change):
        ...
        if mode == "depth":
            self.im.set_clim(
                vmin=self.topo.min_depth, vmax=self.topo.depth.max(skipna=True).item()
            )
    ...
    def _select_cell(self, i, j):
        ...
        vertices = np.array(
            [
                ...
                [
                    self.topo._grid.qlon.data[j, i + 1],
                    self.topo._grid.qlat.data[j, i + 1],
                ],
                [
                    self.topo._grid.qlon.data[j + 1, i + 1],
                    self.topo._grid.qlat.data[j + 1, i + 1],
                ],
                [
                    self.topo._grid.qlon.data[j + 1, i],
                    self.topo._grid.qlat.data[j + 1, i],
                ],
            ]
        )
    ...
    def construct_observances(self):
        ...
        self._min_depth_specifier.observe(
            on_min_depth_change, names="value", type="change"
        )
        ...
        def erase_disconnected_basins(b):
            if self._selected_cell is not None:
                ...
                ocean_mask_changed = np.where(
                    self.topo.basintmask == self.topo.basintmask[j, i], 1, 0
                )
        ...
        self._depth_specifier.observe(on_depth_change, names="value", type="change")
