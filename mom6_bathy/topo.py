...
class Topo:
    ...
    def __init__(self, grid, min_depth, snapshot_dir="Topos", save_on_create=True):
        ...
        if save_on_create:
            ...
            topos_root = os.path.dirname(self.SNAPSHOT_DIR) 
            readme_path = os.path.join(topos_root, "README.md")
            if not os.path.exists(readme_path):
                with open(readme_path, "w") as f:
                    f.write(
                        "# Topos Directory\n\n"
                        "This folder contains bathymetry snapshots generated by "
                        "`mom6_bathy`.\n\n"
                        "Each domain subfolder includes:\n"
                        "- Autosaved .json file for in-memory state\n"
                        "- History file containing undo and redo histories\n"
                        "- The MOM6 grid definition (`grid_*.nc`, `grid_*.json`)\n"
                        "- Saved bathymetry states (`original_topo_*.nc`, snapshots)\n"
                        "- Metadata such as `min_depth`.\n\n"
                        "These files are version-controlled and used to track model "
                        "topographies for MOM6 experiments. Do not edit them manually.\n"
                    )
